...
Nt_fine = length(t_grid);
Nx_fine = 10000;
dx_fine = L/Nx_fine;

x_fine = ((0:Nx_fine-1)+0.5)*dx_fine;
u_array_fine = zeros(Nx_fine,Nt_fine);

for n = 1:Nt_fine
    u_array_fine(:,n) = cosine_interpolation(U_array(:,n),x_fine,L);
end

%% Animation and Snapshots

% Create a subfolder based on the experiment index
saveFolder = sprintf('latex/Images/pstd_snapshots/experiment_%d', exp_idx);
