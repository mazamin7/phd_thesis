function [x_grid,x_gridL,x_gridR,t_grid,u,uL,uR,UL_array,UR_array] = ard_solver_without_nu(u0_fun,v0_fun,f_fun,dx,dt,L,T,c,gamma,nu,bcType,q,r,space_order)
...
function [FcorrL, FcorrR, uL_phys, uR_phys] = get_interface_forces(UL, UR, VL, VR, NxSub, C_res, dx, c, nu)
        uL_phys = idct(UL);
        uR_phys = idct(UR);
        % We don't need v_phys since we drop the nu correction
