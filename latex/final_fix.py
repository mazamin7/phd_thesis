...
if res5 in tex:
    ...
    idx = tex.find(r"\subsection{Smooth propagating pulse (undamped)}")
    if idx != -1:
        tex = tex[:idx+len(r"\subsection{Smooth propagating pulse (undamped)}")] + "\n" + res5 + tex[idx+len(r"\subsection{Smooth propagating pulse (undamped)}"):]
...
if res6 in tex:
    ...
    idx = tex.find(r"\subsection{Non-smooth propagating pulse (undamped)}")
    if idx != -1:
        tex = tex[:idx+len(r"\subsection{Non-smooth propagating pulse (undamped)}")] + "\n" + res6 + tex[idx+len(r"\subsection{Non-smooth propagating pulse (undamped)}"):]
