...
\puttitle{
	...
	course=Mathematical Models and Methods in Engineering - Modelli e Metodi Matematici per l'Ingegneria, % Study Programme (in Italian)
...
Accurate acoustic simulation is important in applications such as
room and architectural acoustics design, Acoustic Virtual Reality,
and sound field reconstruction. In these settings, the acoustic
response may need to be evaluated repeatedly, making the computational
cost of accurate wave-based simulation a major limitation. This thesis
addresses this challenge through the development of efficient numerical
and differentiable methods for the damped viscoelastic wave equation.

A Finite-Difference Time-Domain (FDTD) solver is first introduced for
the damped viscoelastic wave equation. Although the formulation is
standard and does not introduce new numerical techniques, it provides
a baseline for the analysis and validation of the methods developed
subsequently. In this framework, an Optimized Schwarz Waveform
Relaxation (OSWR) method is developed using parameterized Robin-like
transmission conditions. The resulting domain decomposition approach
is investigated within the FDTD framework and provides an effective
iterative coupling between subdomains.

A Pseudo-Spectral Time-Domain (PSTD) solver is then developed for
rectangular domains with homogeneous Neumann boundary conditions,
using an analytic modal propagator. The cosine spectral representation
decouples the spatial problem into independent modal equations, which
are integrated exactly in time for the homogeneous problem. This yields
unconditionally stable temporal propagation without temporal
discretization error in the modal dynamics. Since the resulting PSTD
formulation is not directly compatible with the OSWR domain
decomposition approach considered for FDTD, the Adaptive Rectangular
Decomposition (ARD) method is adopted to overcome the limitations of
the PSTD solver in a decomposed domain. Its coupling mechanism is
derived from the distributional decomposition of the Laplacian and
implemented as a localized explicit interface correction. The
stability and accuracy of higher-order corrections are analyzed.

The resulting modal propagators are finally embedded in a
Differentiable Physics Neural Network (DPNN) for sound field
reconstruction. The framework jointly reconstructs unknown initial
pressure fields and estimates viscoelastic damping parameters from
sparse noisy measurements.

The proposed methods provide complementary strategies for accelerating
accurate wave-based simulations and their repeated use in forward and
inverse acoustic problems.

\textbf{Keywords:} Viscoelastic Wave Equation, Finite-Difference
Time-Domain, Pseudo-Spectral Time-Domain, Domain Decomposition,
Optimized Schwarz Waveform Relaxation, Adaptive Rectangular
Decomposition, Differentiable Physics Neural Networks, Sound Field
...
La simulazione acustica accurata è importante in applicazioni quali
la progettazione dell'acustica degli ambienti e dell'architettura,
l'Acoustic Virtual Reality e la ricostruzione del campo acustico. In
questi contesti, la risposta acustica può dover essere valutata
ripetutamente, rendendo il costo computazionale delle simulazioni
accurate basate sulle onde una delle principali limitazioni. Questa
tesi affronta tale problema attraverso lo sviluppo di metodi numerici
efficienti e differenziabili per l'equazione delle onde
viscoelastiche smorzate.

Viene innanzitutto introdotto un solutore Finite-Difference Time-Domain
(FDTD) per l'equazione delle onde viscoelastiche smorzata. Sebbene la
formulazione sia standard e non introduca nuove tecniche numeriche,
essa fornisce una baseline per l'analisi e la validazione dei metodi
sviluppati successivamente. In questo contesto viene sviluppato un
metodo Optimized Schwarz Waveform Relaxation (OSWR) mediante
condizioni di trasmissione di tipo Robin parametrizzate. Il metodo di
domain decomposition risultante viene studiato nel framework FDTD e
fornisce un coupling iterativo efficace tra i diversi subdomini.

Successivamente viene sviluppato un solutore Pseudo-Spectral
Time-Domain (PSTD) per domini rettangolari con condizioni al contorno
omogenee di Neumann, basato su un propagatore modale analitico. La
rappresentazione spettrale in coseni disaccoppia il problema spaziale
in equazioni modali indipendenti, che vengono integrate esattamente nel
tempo per il problema omogeneo. Ciò consente una propagazione temporale
incondizionatamente stabile, senza errori di discretizzazione
temporale nella dinamica modale. Poiché la formulazione PSTD ottenuta
non è direttamente compatibile con l'approccio di domain decomposition
OSWR considerato per FDTD, il metodo Adaptive Rectangular Decomposition
(ARD) viene adottato per superare le limitazioni del solutore PSTD in
un dominio decomposto. Il suo meccanismo di coupling è derivato dalla
decomposizione distribuzionale del Laplaciano e implementato come una
correzione esplicita localizzata all'interfaccia. Vengono inoltre
analizzate la stabilità e l'accuratezza delle correzioni di ordine
superiore.

Infine, i propagatori modali ottenuti vengono integrati in una
Differentiable Physics Neural Network (DPNN) per la ricostruzione del
campo acustico. Il framework ricostruisce simultaneamente campi di
pressione iniziali incogniti e stima i parametri di smorzamento
viscoelastico a partire da misure sparse e rumorose.

I metodi proposti forniscono strategie complementari per accelerare
simulazioni accurate basate sulle onde e il loro utilizzo ripetuto in
problemi acustici forward e inverse.

\textbf{Keywords:} Equazione delle Onde Viscoelastiche, Finite-Difference
Time-Domain, Pseudo-Spectral Time-Domain, Domain Decomposition,
Optimized Schwarz Waveform Relaxation, Adaptive Rectangular
Decomposition, Differentiable Physics Neural Networks, Ricostruzione
del Campo Sonoro, Problemi Inversi
...
Computational acoustics is the branch of acoustics devoted to solving sound propagation problems using numerical algorithms. The development of efficient and accurate numerical methods for computational acoustics remains, as explained in the following, an open problem.
...
The first computational methods emerged around 1970 with the ray tracing technique, a \emph{geometrical} method. Geometrical acoustics assumes that sound propagates along straight lines (rays) and reflects off surfaces geometrically, much like light in optics. Mathematically, this assumption is justified because, in the high-frequency limit, the wave equation reduces asymptotically to the eikonal equation, whose characteristic curves are precisely these sound rays. Exploiting this framework, the ray tracing technique launches a large number of rays from a source and tracks their specular reflections at the boundaries. This makes the computation of room impulse responses fast (\cite{krokstad_calculating_1968,schroeder_digital_1970,schroeder_computer_1973}) and also enables sound source identification (\cite{luzzato_simple_1986}).

Most classical formulations of the ray tracing technique treat the rays as incoherent. However, treating sound simply as energy rays that sum together ignores its coherent wave nature. In reality, the superposition of sound waves depends heavily on their phase. Without tracking phase information, a model cannot predict whether intersecting waves will constructively amplify or destructively cancel, making it impossible to describe crucial acoustic phenomena like interference effects and standing waves (\cite{funkhouser_beam_1998,kuttruff_acoustics_2007,kuttruff_room_2009}).

Furthermore, the eikonal equation itself does not model diffusion or diffraction, so the ray tracing technique alone ignores these phenomena.

Physically, when an acoustic wave interacts with a rough wall, different behaviors may be observed depending on the size of the irregularities relative to the wavelength of the sound wave. If the irregularities are small compared to the wavelength, the wall behaves approximately as a smooth surface and specular reflection dominates. If they are comparable to the wavelength, diffuse scattering occurs and the sound is redistributed over many directions. If they are large compared to the wavelength, the surface can be described locally by specular reflections from its individual facets or irregular features (\cite{kuttruff_acoustics_2007}).
...
Depending on the specific formulation, the image-source method can be implemented as either a coherent or an incoherent model. Coherent implementations sum the complex acoustic pressures of the virtual sources, thus preserving phase information and modeling interference effects. In contrast, incoherent implementations sum only the energy of the sources, discarding phase and failing to capture wave superposition.

The limitations of both the ray tracing technique and the image-source method in representing diffuse reflections and the reverberation tail were addressed around 1990 by coupling these methods with statistical models and homogeneous reverberant-field approximations, as in \cite{rindel_computer_1992,naylor_odeonanother_1993,maercke_prediction_1993,heinz_binaural_1993}.
...
Gradually, these methods became more efficient and, starting from 1990, were used for modal analysis in room acoustics, although limited to small rooms because of computational limitations (\cite{rindel_computer_1992}).

For room acoustics, the finite speed of sound wave implies that the direct sound and early reflections arrive at clearly distinguishable times in the room impulse response. This makes transient behavior important to model (\cite{funkhouser_beam_1998}).

The first time-dependent wave-based methods appeared around the same period of time, in particular the Finite-Difference Time-Domain (FDTD) method, which is based on discretizing the wave equation in both space and time on a grid. This makes it possible to obtain transient solutions with arbitrarily high accuracy as the grid is refined and the time step is reduced, at the cost of increased computational effort (\cite{alterman_propagation_1968,ottaviani_elastic-wave_1971,alford_accuracy_1974}). This, in turn, makes it possible to compute room impulse responses and energy decay curves, thereby deriving room-acoustic parameters or performing auralization (\cite{botteldooren_finitedifference_1995}).

\subsection{Challenges}
Even as early as 1990, wave-based methods made it possible to compute room impulse responses and perform auralization. However, these methods are still largely limited to small rooms and low frequencies because of computational constraints, as they require the solution of large systems of equations arising from the discretization of the domain. To achieve realistic, real-time simulations for interactive room and architectural acoustics design, entertainment applications such as immersive virtual reality, or inverse problems requiring repeated evaluations, we still rely heavily on geometrical methods (\cite{kleiner_auralization_1993,council_virtual_1995,savioja_creating_1999,sampedro_llopis_reduced_2023}).

There are two possible paths toward efficient and accurate acoustic simulation. One is to improve geometrical methods so they capture phenomena like diffraction and modal resonances. This is usually achieved by augmenting the ray approximation with edge-diffraction heuristics, statistical scattering, or hybrid wave-ray architectures (\cite{calamia_fast_2006,an_diffraction-aware_2018,potter_numerical_2023,yeh_wave-ray_2013,tang_learning_2021}). The second path is to improve the efficiency of purely wave-based methods, which is the focus of this thesis.
%
...
As we have seen, wave-based numerical solvers can be prohibitively expensive. For this reason, a substantial body of literature has been devoted to reducing their computational cost while preserving their accuracy. In this section, we review the main classes of acceleration techniques that are consistent with the methods developed later in this thesis.
%
...
Another approach is Adaptive Rectangular Decomposition (ARD), a numerical solver for the wave equation that partitions the computational domain into non-overlapping rectangular subdomains (\cite{raghuvanshi_efficient_2009}). Within each subdomain, a pseudospectral formulation is employed: the solution is expanded in a discrete cosine transform of type II (DCT-II) basis defined on a staggered uniform Cartesian grid (\cite{rao_discrete_1990}), and the evolution problem is advanced using the exact time-marching scheme presented in \cite{cieslinski_exact_2011}. The basis functions satisfy homogeneous Neumann boundary conditions by construction, making them well suited to the local subdomain problems. To model partial or total absorption at the outer boundaries, ARD couples this pseudospectral discretization with a perfectly matched layer (PML).

The advantage of ARD is that, provided that the spatial discretization adequately resolves the solution, the subdomain evolution is propagated without additional time-discretization error, which makes the method particularly attractive for our purposes. The only source of error comes from the subdomain communication scheme.
...
In this chapter, we derive the continuous properties of the damped wave equation and formulate numerical methods to simulate it accurately and efficiently, namely the Finite-Difference Time-Domain (FDTD) method and the Pseudo-Spectral Time-Domain (PSTD) method.
...
To model a general attenuation law consistent with the atmospheric absorption formulas summarized in the ISO standard \cite{iso_acoustics_1993}, one may consider an acoustic propagation equation with multiple relaxation terms, as proposed in \cite{nachman_equation_1990}. Assuming plane-wave solutions leads to a dispersion relation of kind
\begin{equation*}
k^2(\omega)
=
\omega^2\left(
...
=0,
\end{equation*}
where $u$ denotes the wave field, $c$ is the wave speed [m/s], $\gamma$ the telegrapher damping coefficient [1/s], and $\nu$ the viscoelastic damping coefficient [m$^2$/s].
...
x\in(0,L),\quad t>0,
\end{equation}
where \(u(x,t)\) denotes the wave field, \(c>0\) is the wave propagation velocity [m/s], \(\gamma\ge0\) is the coefficient of telegrapher damping [1/s], and \(\nu\ge0\) is the coefficient of viscoelastic damping [m$^2$/s]. The source term is
denoted by \(f(x,t)\).
...
-i\nu\omega k^2u.
\end{equation*}
Since \(u\neq0\), dividing the above equation by \(u\) yields
\begin{equation*}
...
In this section, we define a fixed set of six benchmark problems that are reused throughout the thesis to evaluate different numerical methods.

\subsection{Common Experimental Setup}
Unless otherwise specified, the computational domain is $\Omega=(0,L)$ and homogeneous Neumann boundary conditions are imposed, $\partial_xu(0,t)=\partial_xu(L,t)=0$. The initial velocity and forcing are identically zero, $\partial_tu(x,0)=0$, $f(x,t)=0$. Simulations are run up to a final time $T=2$.
...
\subsubsection{Configuration II: Non-smooth standing wave (undamped)}
The initial displacement is a trapezoidal profile, given by the piecewise linear function
\[
u(x,0) = 
\begin{cases}
1 & \text{if } 0 \le x \le \frac{L}{4}, \\[4pt]
2 - \frac{4x}{L} & \text{if } \frac{L}{4} < x < \frac{3L}{4}, \\[4pt]
-1 & \text{if } \frac{3L}{4} \le x \le L,
\end{cases}
\]
which is equivalent to the cosine expansion
\[
u(x,0)
=
\sum_{j=0}^{\infty}
...
\subsubsection{Configuration III: Smooth standing wave (damped)}
...
U_m(t)\phi_m(x),
\]
where the modal amplitudes now include the effect of telegrapher and viscoelastic damping.

\subsubsection{Configuration IV: Non-smooth standing wave (damped)}
...
\nu=0.005.
\]
The initial displacement is the same trapezoidal profile considered in Experiment 2,
\[
u(x,0) = 
\begin{cases}
1 & \text{if } 0 \le x \le \frac{L}{4}, \\[4pt]
...
\end{cases}
\]
which corresponds to the cosine expansion
\[
u(x,0)
=
\sum_{j=0}^{\infty}
...
f(x,t)=0.
\]
The reference solution is computed from the cosine expansion of the initial condition, with each modal coefficient propagated analytically according to the damped modal dynamics. As in the undamped case, the modal expansion is truncated after a sufficiently large number of modes.

\subsubsection{Configuration V: Smooth propagating pulse (undamped)}
...
The reference solution is obtained by applying d'Alembert's formula to the even $2L$-periodic extension of the initial data \cite[Sec.~4.2]{olver_introduction_2014}. More precisely, the initial condition is extended to an even $2L$-periodic function, $u_0^{\mathrm{ext}}(x)$, which automatically satisfies the homogeneous Neumann boundary conditions. The exact solution is therefore
\[
u(x,t)
=
\frac12
...
u_0^{\mathrm{ext}}(x+ct).
\]
Physically, this even periodic extension can be viewed as an infinite set of replicas located at
\[
x=2mL\pm\mu,
\qquad
m\in\mathbb Z.
\]
As observed by \citet[Sec.~4.2]{olver_introduction_2014}, when a pulse hits one of the ends, its reflection remains upright but becomes a mirror image of the original, preserving the sign of the wave. This construction exactly reproduces the multiple reflections induced by the homogeneous Neumann boundary conditions. 

\subsubsection{Configuration VI: Non-smooth propagating pulse (undamped)}
...
\gamma=\nu=0.
\]
The initial displacement is a triangular pulse centered at $\mu = L/4$ with support determined by $\sigma = L/50$, explicitly given by
\[
u(x,0) = \max\left(1 - \frac{|x-\mu|}{\sigma}, 0\right),
\]
while the initial velocity and forcing vanish,
\[
\partial_tu(x,0)=0,
\qquad
f(x,t)=0.
...
As in the previous experiment, the reference solution is obtained by applying d'Alembert's formula to the even $2L$-periodic extension of the initial data \cite[Sec.~4.2]{olver_introduction_2014}. The initial triangular profile is extended to an even $2L$-periodic function, $u_0^{\mathrm{ext}}(x)$, yielding the exact solution
\[
u(x,t)
=
\frac12
...
u_0^{\mathrm{ext}}(x+ct).
\]
Physically, this even periodic extension corresponds to an infinite set of replicas located at
\[
x=2mL\pm\mu,
\qquad
m\in\mathbb Z.
\]
As noted in \cite[Sec.~4.2]{olver_introduction_2014}, when a pulse hits one of the ends it reflects as an upright mirror image, preserving the sign of the wave. This construction exactly reproduces the multiple reflections induced by the homogeneous Neumann boundary conditions. 
...
We consider the damped wave equation \eqref{eq:wave} on a node-centered grid defined by
\[
x_i=i\Delta x, \quad \text{for } i = 0, \dots, N, \qquad t_n=n\Delta t, \quad \text{for } n \ge 0.
\]
We denote the numerical approximation of $u$ in $x_i$ at time $t_n$ by
\[
...
\]
denotes the prescribed initial velocity inside our domain.
...
By collecting the unknowns at all grid points $i=0,\dots,N$ into the global state vector $\mathbf{u}^n = [u_0^n, \dots, u_N^n]^T$, the interior update equations for $i=1,\dots,N-1$ can be expressed in matrix form as
\begin{equation}
A_{\mathrm{int}}\mathbf{u}^{n+1}
=
B_{\mathrm{int}}\mathbf{u}^{n}
+
C_{\mathrm{int}}\mathbf{u}^{n-1}
+
\Delta t^2\mathbf{f}^{n},
\label{eq:fdtd_interior_system}
\end{equation}
where $A_{\mathrm{int}}, B_{\mathrm{int}}$, and $C_{\mathrm{int}}$ are $(N+1)\times(N+1)$ tridiagonal matrices. Multiplying the scalar scheme \eqref{eq:fdtd_scalar} by $\Delta t^2$ and isolating the time levels yields their explicit coefficients:
\begin{align*}
A_{\mathrm{int}}
&=
\left(1+\frac{\gamma\Delta t}{2}\right)\mathbf{I}
-
\frac{\nu\Delta t}{2\Delta x^2} \mathbf{D}_2,
\\
B_{\mathrm{int}}
&=
2\mathbf{I}
+
\frac{c^2\Delta t^2}{\Delta x^2} \mathbf{D}_2,
\\
C_{\mathrm{int}}
&=
-\left(1-\frac{\gamma\Delta t}{2}\right)\mathbf{I}
...
\frac{\nu\Delta t}{2\Delta x^2} \mathbf{D}_2,
\end{align*}
where $\mathbf{I}$ is the identity matrix and $\mathbf{D}_2 = \operatorname{tridiag}(1, -2, 1)$ is the standard discrete Laplacian matrix.

To enforce boundary conditions, the first and last equations of this algebraic system are modified, leading to the global system
\begin{equation}
A_{\mathrm{bc}}\mathbf{u}^{n+1}
=
B_{\mathrm{bc}}\mathbf{u}^{n}
+
C_{\mathrm{bc}}\mathbf{u}^{n-1}
+
\Delta t^2\mathbf{f}^{n}
+
\mathbf{b}_{\mathrm{bc}}^{\,n}.
\label{eq:fdtd_global_system}
\end{equation}
Here, $A_{\mathrm{bc}}, B_{\mathrm{bc}}$, and $C_{\mathrm{bc}}$ coincide with $A_{\mathrm{int}}, B_{\mathrm{int}}$, and $C_{\mathrm{int}}$ respectively, except for their first and last rows, which are populated by the specific boundary scheme, and the vector $\mathbf{b}_{\mathrm{bc}}^{\,n}$ contains the prescribed boundary data.
...
\\
u_{N}^{n+1}&=g_{\mathrm R}(t^{n+1}).
\end{align*}

Accordingly, the first and last rows of the matrices are modified as follows. The first and last rows of $A_{\mathrm{bc}}$ are replaced by the corresponding canonical basis vectors:
\begin{equation*}
[A_{\mathrm{bc}}]_{0,j} = \delta_{0,j}, \qquad [A_{\mathrm{bc}}]_{N,j} = \delta_{N,j},
\end{equation*}
where $\delta_{i,j}$ is the Kronecker delta. The corresponding rows of $B_{\mathrm{bc}}$ and $C_{\mathrm{bc}}$ are set to zero:
\begin{equation*}
[B_{\mathrm{bc}}]_{0,j} = [B_{\mathrm{bc}}]_{N,j} = 0, \qquad [C_{\mathrm{bc}}]_{0,j} = [C_{\mathrm{bc}}]_{N,j} = 0.
\end{equation*}
The boundary contribution vector takes the prescribed values at the first and last entries, and zero elsewhere:
\begin{equation*}
\mathbf{b}_{\mathrm{bc}}^{\,n}
=
\begin{bmatrix}
...
\end{align*}
ghost nodes are introduced outside the computational domain and eliminated through the second-order accurate centered approximations
\begin{equation*}
...
Substituting these relations into the interior scalar scheme \eqref{eq:fdtd_scalar} evaluated at the boundary nodes $i=0$ and $i=N$ effectively eliminates the ghost nodes. The modified first and last rows of the matrices explicitly become:
\begin{align*}
[A_{\mathrm{bc}}]_{0, :} &= \left[ 1+\frac{\gamma\Delta t}{2}+\frac{\nu\Delta t}{\Delta x^2}, -\frac{\nu\Delta t}{\Delta x^2}, 0, \dots, 0 \right], \\
[B_{\mathrm{bc}}]_{0, :} &= \left[ 2-\frac{2c^2\Delta t^2}{\Delta x^2}, \frac{2c^2\Delta t^2}{\Delta x^2}, 0, \dots, 0 \right], \\
[C_{\mathrm{bc}}]_{0, :} &= \left[ -\left(1-\frac{\gamma\Delta t}{2}\right) + \frac{\nu\Delta t}{\Delta x^2}, -\frac{\nu\Delta t}{\Delta x^2}, 0, \dots, 0 \right],
\end{align*}
and
\begin{align*}
[A_{\mathrm{bc}}]_{N, :} &= \left[ 0, \dots, 0, -\frac{\nu\Delta t}{\Delta x^2}, 1+\frac{\gamma\Delta t}{2}+\frac{\nu\Delta t}{\Delta x^2} \right], \\
[B_{\mathrm{bc}}]_{N, :} &= \left[ 0, \dots, 0, \frac{2c^2\Delta t^2}{\Delta x^2}, 2-\frac{2c^2\Delta t^2}{\Delta x^2} \right], \\
[C_{\mathrm{bc}}]_{N, :} &= \left[ 0, \dots, 0, -\frac{\nu\Delta t}{\Delta x^2}, -\left(1-\frac{\gamma\Delta t}{2}\right) + \frac{\nu\Delta t}{\Delta x^2} \right].
\end{align*}

The resulting boundary contribution incorporating the source terms is
\begin{equation*}
\mathbf{b}_{\mathrm{bc}}^{\,n}
=
...
\frac{\nu\Delta t}{\Delta x}
\begin{bmatrix}
g_{\mathrm L}(t^{n+1}) - g_{\mathrm L}(t^{n-1}) \\
0 \\
\vdots \\
0 \\
g_{\mathrm R}(t^{n+1}) - g_{\mathrm R}(t^{n-1})
\end{bmatrix}.
\end{equation*}
...
of the form
\begin{equation*}
    u_j^n=\xi^n e^{\mathrm{i}kj\Delta x},
\end{equation*}
...
u_j^{n+1}&=\xi u_j^n,\\
u_j^{n-1}&=\xi^{-1}u_j^n,\\
u_{j\pm1}^{\,n}&=e^{\pm\mathrm{i}k\Delta x}u_j^n,\\
u_{j\pm1}^{\,n\pm1}&=\xi^{\pm1}e^{\pm\mathrm{i}k\Delta x}u_j^n.
\end{align*}
Consequently,
\begin{align*}
u_{j+1}^n-2u_j^n+u_{j-1}^n
&=
\left(e^{\mathrm{i}k\Delta x}-2+e^{-\mathrm{i}k\Delta x}\right)u_j^n\\
&=
2(\cos(k\Delta x)-1)u_j^n\\
&=
-2\delta\,u_j^n,
\end{align*}
where
...
Since the polynomial has real coefficients and $a>0$, the Jury criterion (Cfr. \cite{jury_theory_1973}, Chapter~3, \S3.5, equations~(3.14), (3.21), and~(3.46)–(3.48)) states
...
\begin{proposition}[Convergence of the piecewise linear interpolation]
Let $u(\cdot,t) \in H^1(0,L)$ for all $t > 0$, and let
\[
I_h u(\cdot,t) : H^1(0,L) \to V_h
\]
denote its continuous piecewise linear interpolant on a uniform mesh of size $h = \Delta x$. Then
\[
I_h u(\cdot,t) \to u(\cdot,t) \quad \text{in } L^2(0,L),
\]
as $h \to 0$. Moreover, if $u(\cdot,t) \in H^2(0,L)$ for all $t > 0$, there exists a constant $C$, independent of $h$, such that
\[
\|u(\cdot,t) - I_h u(\cdot,t)\|_{L^2(0,L)} \le Ch^2 |u(\cdot,t)|_{H^2(0,L)}.
\]
More generally, if $u(\cdot,t) \in H^r(0,L)$ for $1 \le r \le 2$, then
\[
\|u(\cdot,t) - I_h u(\cdot,t)\|_{L^2(0,L)} = O(h^r).
\]
\end{proposition}

\begin{proof}
These are classical interpolation estimates for continuous piecewise linear finite elements. A proof can be found, for example, in \cite{quarteroni_numerical_2007}, Chapter~8, where interpolation error estimates for finite-element spaces are derived using the Bramble--Hilbert lemma.
\end{proof}
...
\subsection{Staggered FDTD Discretization and Modal Expansion with Homogeneous Neumann Boundary Conditions}
...
\Delta t^2f^n
\end{equation*}
denote the interior scheme \eqref{eq:fdtd_global_system} introduced previously.
...
S_m
&=
e^{A_m\Delta t}, \label{eq:Sm_def}
\\
T_m
&=
\int_0^{\Delta t}
e^{A_m(\Delta t-\tau)}
\,d\tau, \label{eq:Tm_def}
\end{align}
...
\begin{equation*}
    ...
    T\mathbf W^n.
\end{equation*}
Employing the permuted vectors to group the modal displacements and velocities block-wise, we can rewrite it as
\begin{equation*}
...
\mathbf W_m^{\,n},
\end{equation*}
is unconditionally stable, where $S_m$ and $T_m$ are defined in \eqref{eq:Sm_def} and \eqref{eq:Tm_def}. More precisely,
\begin{equation*}
\rho(S_m)\le1,
\end{equation*}
...
\end{equation*}
be the exact evolution of the modal coefficients over one time step, and let
\begin{equation*}
\hat{\mathbf Z}_m^{\,n+1}
=
S_m\mathbf Z_m^n
...
O(\Delta t)
\end{equation*}
as shown in \cite{quarteroni_numerical_2014}.
\end{proposition}
...
Since $A_m$ is constant, the matrix exponential is uniformly bounded over finite intervals \cite{quarteroni_numerical_2014}. Thus, denoting by $\|\cdot\|$ the standard vector 2-norm and its induced matrix norm, there exists a constant $C_m>0$ such that
\begin{equation*}
\left\|
e^{A_ms}
\right\|
...
\)
the matrix dispersion relation \eqref{eq:matrix_dispersion} converges to the continuous dispersion relation \eqref{eq:dispersion},
...
Assume now that the exact solution belongs to $V_N$.
...
Let $u(\cdot,t) \in L^2(0,L)$ for all $t > 0$, with cosine expansion
\[
u(x,t)
=
\sum_{m=0}^{\infty}
...
a_m(t)\phi_m(x)
\]
denote its projection onto the approximation space $V_N = \operatorname{span}\{\phi_0,\ldots,\phi_{N-1}\}$.
Then,
\[
P_N u(\cdot,t)\longrightarrow u(\cdot,t)
\qquad\text{in }L^2(0,L),
\]
as $N\rightarrow\infty$.

Moreover, if $u(\cdot,t)\in H^r(0,L)$, then
\[
\|u(\cdot,t)-P_N u(\cdot,t)\|_{L^2(0,L)}
=
O(N^{-r})
=
O(\Delta x^{\,r}),
\]
whereas, if $u(\cdot,t)$ is analytic, there exist constants $C,\alpha>0$ such that
\[
\|u(\cdot,t)-P_N u(\cdot,t)\|_{L^2(0,L)}
\le
Ce^{-\alpha N}
...
For a general solution, Proposition~\ref{prop:cosine_approximation_convergence} implies that the cosine approximation converges to the exact solution as $\Delta x\to0$. Consequently, the matrix dispersion relation \eqref{eq:matrix_dispersion} converges to the continuous dispersion relation \eqref{eq:dispersion} with the same algebraic or spectral rate as the underlying cosine approximation.
...
\right)^{1/2}.
\]
This metric measures the global discrepancy, encompassing both the projection error of the initial condition and the accumulation of numerical errors during the time integration.

For the FDTD method we choose $\Delta t = \Delta x/c$ (which corresponds to the stability limit), while for the PSTD method we choose $\Delta t = 0.9\Delta x/c$ (this choice avoids the `magic step' where $\text{CFL}=1$ gives superconvergence, ensuring a fair evaluation of the generic numerical dispersion rather than a special-case exactness).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Smooth Standing Wave (Undamped)}
This benchmark verifies the accuracy of both schemes for smooth standing-wave solutions. In one spatial dimension, when the CFL condition satisfies $c\Delta t=\Delta x$, the undamped FDTD scheme reduces to the leapfrog method and exhibits the expected exponential convergence of the discretization error (\cite{bilbao_numerical_2009}). Since the exact solution is entirely contained in the span of the retained cosine modes, the PSTD method reproduces the spatial solution exactly and the numerical error is reduced to machine precision.

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_1/snapshot_t_0.2000.png} \caption{FDTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_1/snapshot_t_0.5000.png} \caption{FDTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_1/snapshot_t_1.0000.png} \caption{FDTD $t=1.0$} \end{subfigure} \\
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_1/snapshot_t_0.2000.png} \caption{PSTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_1/snapshot_t_0.5000.png} \caption{PSTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_1/snapshot_t_1.0000.png} \caption{PSTD $t=1.0$} \end{subfigure}
    \caption{Snapshots of the smooth undamped standing wave.}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_1/spacetime_solution.png} \caption{FDTD spacetime.} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_1/spacetime_solution.png} \caption{PSTD spacetime.} \end{subfigure}
...
\toprule
& \multicolumn{5}{c|}{\textbf{FDTD Error ($L_2$)}} & \multicolumn{5}{c}{\textbf{PSTD Error ($L_2$)}} \\
\textbf{Exp} & $N=100$ & $N=200$ & $N=400$ & $N=800$ & \textbf{Rate} & $N=100$ & $N=200$ & $N=400$ & $N=800$ & \textbf{Rate} \\
\midrule
  1 & 5.55e-06 & 1.34e-06 & 3.33e-07 & 3.63e-08 & 3.20 & 1.33e-15 & 1.01e-14 & 2.43e-14 & 2.86e-14 & - \\
  2 & 3.31e-03 & 1.58e-03 & 8.03e-04 & 3.93e-04 & 1.03 & 9.06e-05 & 1.28e-04 & 1.81e-05 & 1.46e-05 & 0.31 \\
  3 & 1.76e-05 & 4.39e-06 & 1.09e-06 & 2.89e-07 & 1.92 & 6.59e-15 & 1.23e-14 & 1.19e-14 & 4.78e-14 & - \\
  4 & 5.29e-05 & 1.26e-05 & 2.78e-06 & 5.97e-07 & 2.22 & 2.39e-05 & 6.05e-06 & 1.36e-06 & 4.55e-07 & 1.58 \\
  5 & 2.44e-04 & 4.05e-05 & 8.92e-06 & 2.16e-06 & 2.05 & 9.81e-16 & 2.86e-15 & 5.37e-15 & 1.01e-14 & - \\
  6 & 2.11e-02 & 1.46e-02 & 6.56e-03 & 3.46e-03 & 0.92 & 2.78e-04 & 3.93e-04 & 5.46e-04 & 8.19e-05 & 2.74 \\
\bottomrule
\end{tabular}%
}
\caption{Convergence analysis for the various experimental configurations. The values represent the global $L_2$ discretization error at the final time $T=2$.}
...
This experiment investigates the effect of reduced spatial regularity on the convergence of the two methods. As in the previous experiment, the FDTD method exhibits the expected exponential convergence of the discretization error. Since the cosine coefficients of non-smooth functions decay algebraically rather than exponentially, the PSTD method exhibits algebraic convergence.

\begin{figure}[H]
    \centering
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_2/snapshot_t_0.2000.png} \caption{FDTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_2/snapshot_t_0.5000.png} \caption{FDTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_2/snapshot_t_1.0000.png} \caption{FDTD $t=1.0$} \end{subfigure} \\
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_2/snapshot_t_0.2000.png} \caption{PSTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_2/snapshot_t_0.5000.png} \caption{PSTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_2/snapshot_t_1.0000.png} \caption{PSTD $t=1.0$} \end{subfigure}
    \caption{Snapshots of the non-smooth undamped standing wave.}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_2/spacetime_solution.png} \caption{FDTD spacetime.} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_2/spacetime_solution.png} \caption{PSTD spacetime.} \end{subfigure}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_3/snapshot_t_0.2000.png} \caption{FDTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_3/snapshot_t_0.5000.png} \caption{FDTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_3/snapshot_t_1.0000.png} \caption{FDTD $t=1.0$} \end{subfigure} \\
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_3/snapshot_t_0.2000.png} \caption{PSTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_3/snapshot_t_0.5000.png} \caption{PSTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_3/snapshot_t_1.0000.png} \caption{PSTD $t=1.0$} \end{subfigure}
    \caption{Snapshots of the smooth damped standing wave.}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_3/spacetime_solution.png} \caption{FDTD spacetime.} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_3/spacetime_solution.png} \caption{PSTD spacetime.} \end{subfigure}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_4/snapshot_t_0.2000.png} \caption{FDTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_4/snapshot_t_0.5000.png} \caption{FDTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_4/snapshot_t_1.0000.png} \caption{FDTD $t=1.0$} \end{subfigure} \\
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_4/snapshot_t_0.2000.png} \caption{PSTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_4/snapshot_t_0.5000.png} \caption{PSTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_4/snapshot_t_1.0000.png} \caption{PSTD $t=1.0$} \end{subfigure}
    \caption{Snapshots of the non-smooth damped standing wave.}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_4/spacetime_solution.png} \caption{FDTD spacetime.} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_4/spacetime_solution.png} \caption{PSTD spacetime.} \end{subfigure}
...
This experiment evaluates the propagation accuracy of the two schemes under repeated Neumann reflections. As in Experiment~1, the FDTD method exhibits the expected exponential convergence of the discretization error. Although the Gaussian pulse is not exactly represented by a finite cosine expansion, its cosine coefficients decay exponentially. Consequently, the PSTD method rapidly reaches machine precision.
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_5/snapshot_t_0.2000.png} \caption{FDTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_5/snapshot_t_0.5000.png} \caption{FDTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_5/snapshot_t_1.0000.png} \caption{FDTD $t=1.0$} \end{subfigure} \\
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_5/snapshot_t_0.2000.png} \caption{PSTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_5/snapshot_t_0.5000.png} \caption{PSTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_5/snapshot_t_1.0000.png} \caption{PSTD $t=1.0$} \end{subfigure}
    \caption{Snapshots of the Gaussian pulse.}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_5/spacetime_solution.png} \caption{FDTD spacetime.} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_5/spacetime_solution.png} \caption{PSTD spacetime.} \end{subfigure}
...
This benchmark evaluates the behavior of the two methods for propagating solutions with limited spatial regularity and sharp gradients. The FDTD method again exhibits the expected exponential convergence of the discretization error. Since the solution is non-smooth, the cosine coefficients decay only algebraically, leading the PSTD method to exhibit algebraic convergence.
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_6/snapshot_t_0.2000.png} \caption{FDTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_6/snapshot_t_0.5000.png} \caption{FDTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_6/snapshot_t_1.0000.png} \caption{FDTD $t=1.0$} \end{subfigure} \\
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_6/snapshot_t_0.2000.png} \caption{PSTD $t=0.2$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_6/snapshot_t_0.5000.png} \caption{PSTD $t=0.5$} \end{subfigure}
    \begin{subfigure}{0.3\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_6/snapshot_t_1.0000.png} \caption{PSTD $t=1.0$} \end{subfigure}
    \caption{Snapshots of the triangular pulse.}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{fdtd_snapshots/experiment_6/spacetime_solution.png} \caption{FDTD spacetime.} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{pstd_snapshots/experiment_6/spacetime_solution.png} \caption{PSTD spacetime.} \end{subfigure}
...
To solve the damped wave equation on large computational domains, we employ an overlapping Schwarz Waveform Relaxation (SWR) method. For the classical Schwarz Waveform Relaxation method, for which Dirichlet transmission conditions are employed, whereas for Optimized Schwarz Waveform Relaxation (OSWR) methods, we develop transmission operators that minimize the convergence factor by employing Robin-like transmission conditions depending on two parameters $q$ and $r$. As the PSTD method only supports homogeneous Neumann conditions, we employ SWR only for FDTD.

\subsection{SWR Algorithm}
Let $\Omega=(0,L)$ be decomposed into the two overlapping subdomains $\Omega_1=(0,b)$ and $\Omega_2=(a,L)$, with
\[
0<a<b<L,
\qquad \text{and} \qquad
\delta=b-a>0,
\]
where $\delta$ denotes the overlap width. Let $u$ and $v$ denote the solutions defined on the left ($\Omega_1$) and right ($\Omega_2$) subdomains, respectively.
...
The real numbers $q,r\ge0$ are the transmission parameters that we want to optimize.

\subsection{Convergence Factor}

To analyze the convergence of the algorithm, it is sufficient to consider the error equations. Since the exact solution satisfies the same equations, the forcing term, initial conditions and physical boundary data vanish. We therefore consider
...
(c^2+\nu s)\widehat{u}_{xx}
=
(s^2+\gamma s)\widehat{u}, \qquad s \in \mathbb{C}, \ x \in (0,L).
\end{equation*}
...
Substituting into the Laplace-transformed Robin transmission conditions, we get that
...
We compare the convergence of the SWR iteration using classical Robin transmission conditions ($q=1/c$, $r=0$) against the optimized Robin parameters derived from the theoretical min-max problem (Equation~\ref{eq:minmax}). Figure~\ref{fig:oswr_convergence} illustrates the $L^\infty$ error over successive Schwarz iterations, measured against the monolithic FDTD reference solution. The theoretically optimized Robin conditions do not substantially accelerate the convergence rate. As demonstrated in the subsequent experiments, this limited improvement occurs because a low viscoelastic damping coefficient was chosen for the current configurations.

\begin{figure}[H]
     \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/fdtd_snapshots/experiment_1_swr/swr_convergence.png} \caption{Smooth standing wave (undamped)} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/fdtd_snapshots/experiment_3_swr/swr_convergence.png} \caption{Smooth standing wave (damped)} \end{subfigure} \\
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/fdtd_snapshots/experiment_2_swr/swr_convergence.png} \caption{Non-smooth standing wave (undamped)} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/fdtd_snapshots/experiment_4_swr/swr_convergence.png} \caption{Non-smooth standing wave (damped)} \end{subfigure} \\
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/fdtd_snapshots/experiment_5_swr/swr_convergence.png} \caption{Smooth propagating pulse (undamped) \\ ~} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/fdtd_snapshots/experiment_6_swr/swr_convergence.png} \caption{Non-smooth propagating pulse (undamped)} \end{subfigure}
    \caption{Convergence of the SWR iteration for classical vs optimized Robin transmission conditions under six different initial conditions. In all configurations, the theoretically optimized parameters strictly bound the contraction rate and consistently provide a faster contraction of the error per iteration compared to the non-optimized parameters $q=1/c$ and $r=0$.}
    \label{fig:oswr_convergence}
\end{figure}

To better characterize the performance of the Optimized Schwarz Waveform Relaxation method, we investigate the influence of the viscoelastic damping parameter $\nu$ on the optimized Robin parameters. For these evaluations, we consider the non-smooth standing wave configuration as our baseline setup. The simulation duration is set to $T=2$.

Figure~\ref{fig:oswr_nu_sweep} illustrates the behavior of the SWR convergence across varying levels of viscosity $\nu$. As the viscosity increases, the optimal transmission parameters deviate more significantly from the classical Robin conditions ($q=1/c$, $r=0$).
...
To provide a broader overview of how both damping parameters jointly affect the domain decomposition performance, Figure~\ref{fig:oswr_rho_opt_map} illustrates the optimal continuous convergence factor $\rho_{\mathrm{opt}}$ computed across a 2D parameter space varying both the baseline Telegrapher damping $\gamma$ and the viscoelastic damping $\nu$. The map evaluates $\rho_{\mathrm{opt}}$ by sweeping $\gamma \in [0, 5]$ and $\nu \in [10^{-3}, 10^{1}]$ over a $30 \times 30$ grid. The other physical and numerical parameters remain fixed at their baseline values ($c=1$, $L=1$, $T=2$, $\Delta t=0.05$, and overlap $\delta=L/10$).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/fdtd_snapshots/swr_parameters/swr_rho_opt_map.png}
    \caption{2D map of the optimal continuous convergence factor $\rho_{\mathrm{opt}}$ as a function of the Telegrapher damping $\gamma$ and the viscoelastic damping $\nu$.}
    \label{fig:oswr_rho_opt_map}
\end{figure}

To further understand the behavior of the min-max optimization, we can examine how the optimal Robin parameters $(q_{\mathrm{opt}}, r_{\mathrm{opt}})$ vary as a mapping from the physical damping parameters $\gamma$ and $\nu$. Figure~\ref{fig:swr_qr_mapping_all} presents a scatter plot of the optimal parameters computed over the entire parameter space $\gamma \in [0, 5]$ and $\nu \in [10^{-3}, 10^{1}]$.

To better distinguish the individual effects of the Telegrapher damping $\gamma$ and the viscoelastic damping $\nu$, we parameterize the trajectories of the optimal $(q,r)$ values. Figure~\ref{fig:swr_qr_mapping_gamma_curves} displays the trajectories obtained by holding $\gamma$ constant at discrete levels while continuously sweeping $\nu$. The color of each dot indicates the associated value of $\nu$ on a logarithmic scale. Conversely, Figure~\ref{fig:swr_qr_mapping_nu_curves} illustrates the complementary set of curves formed by holding $\nu$ constant while sweeping $\gamma$, with the color indicating the value of $\gamma$. This parametric mapping demonstrates that $\nu$ and $\gamma$ exert distinct, non-degenerate influences on the optimal Robin parameters.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{Images/fdtd_snapshots/swr_parameters/swr_qr_mapping_all.png}
    \caption{Mapping of the optimal parameters $(q, r)$ across the dense grid of $(\gamma, \nu)$.}
    \label{fig:swr_qr_mapping_all}
\end{figure}

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{Images/fdtd_snapshots/swr_parameters/swr_qr_mapping_gamma_curves.png} \caption{Constant $\gamma$ curves sweeping $\nu$.} \label{fig:swr_qr_mapping_gamma_curves} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{Images/fdtd_snapshots/swr_parameters/swr_qr_mapping_nu_curves.png} \caption{Constant $\nu$ curves sweeping $\gamma$.} \label{fig:swr_qr_mapping_nu_curves} \end{subfigure}
    \caption{Parametric trajectories of the optimal parameters $(q, r)$ in response to continuous variations of a single physical damping parameter.}
    \label{fig:swr_qr_mapping_curves}
\end{figure}

To definitively validate the theoretical min-max optimization problem (Equation~\ref{eq:minmax}), we perform an exhaustive numerical sweep over a 2D grid of Robin parameters $(q, r)$. For this visualization, we explicitly configure the non-smooth standing wave experiment with a baseline Telegrapher damping of $\gamma=0.5$ and a highly viscous damping coefficient ($\nu=2$) to ensure the optimal parameter basin is clearly pronounced. All other settings ($c=1$, $L=1$, $T=2$, $\Delta x=\Delta t=0.05$, $\delta=L/10$) are preserved. For each $(q,r)$ configuration, we run the SWR iteration and extract the empirical maximum error after a fixed number of iterations. We then evaluate the analytical convergence factor $\rho$ over the same parameter space. Figure~\ref{fig:oswr_empirical_vs_theoretical} compares the empirically measured optimal parameters against the analytically predicted continuous optimum. The location of the minimum of the convergence factor lies in a region of minima of the empirical error, confirming the validity of the analytical derivation and the resulting parameter optimization framework.

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{Images/fdtd_snapshots/swr_parameters/swr_empirical_error.png} \caption{Measured empirical error.} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=\textwidth]{Images/fdtd_snapshots/swr_parameters/swr_predicted_rho.png} \caption{Predicted max convergence factor.} \end{subfigure}
    \caption{Comparison of the empirically measured optimum error versus the theoretical continuous optimum over the parameter space $(q,r)$.}
    \label{fig:oswr_empirical_vs_theoretical}
\end{figure}

\subsection{Extension to Two-Dimensional Domains}
...
To definitively validate this extended theoretical framework, we implement the overlapping decomposition for the FDTD method on a 2D Cartesian grid. The subdomains exchange data via the second-order BDF Robin discretization applied independently along each row of the artificial interfaces. The experiment is conducted on a $\Omega = [0, \pi] \times [0, \pi]$ domain discretized with a $30 \times 30$ grid. The domain is split into two overlapping subdomains with an overlap size of $\delta = 2 \Delta x$. To rigorously test the robustness of the optimized parameters across difficult dissipative regimes, we configure the simulation with a significant viscoelastic damping factor ($\nu = 1.0$) alongside a baseline telegrapher damping ($\gamma = 0.5$). We employ as initial conditions a superposition of several high-frequency spatial modes of the form $\sum_{m,n} (mn)^{-2} \sin(m x) \sin(n y)$.

Figure~\ref{fig:oswr_convergence_comparison_2d} tracks the global $L_\infty$ error as a function of the Schwarz iteration count. It compares the convergence history of the unoptimized classical Robin transmission conditions ($q=1/c, r=0$) against the optimally tuned parameters $q^\ast$ and $r^\ast$. Utilizing the numerically optimized transmission condition parameters significantly accelerates the solver convergence.
\begin{figure}[H]
    \centering
...
To understand the ARD coupling mechanism, we turn to the immersed-layer distributional framework introduced in~\cite{eldredge_method_2022}. This framework provides a rigorous way to interpret discrete domain decomposition methods by formulating them as a single continuous partial differential equation across the entire domain, using singular distributions (such as Dirac deltas and their derivatives) to model the jump conditions and coupling forces at the artificial interfaces. For simplicity, consider the one-dimensional domain
\[
\Omega=[0,L]
\]
...
Let $u$ denote the exact continuous solution. To formalize the decomposition, we introduce the overline notation $\overline{(\cdot)}$ to denote the piecewise assembly of a function or its classical derivatives from the independent subdomain solutions. Thus, $\bar{u}$ is the piecewise field obtained by solving independently on $\Omega_1$ and $\Omega_2$, while $\overline{u_x}$ and $\overline{u_{xx}}$ represent the piecewise fields constructed from their classical first and second spatial derivatives, respectively. Conversely, a derivative subscript applied outside the bar, such as $\bar{u}_x$ or $\bar{u}_{xx}$, denotes the global distributional derivative of the assembled piecewise field $\bar{u}$ over the entire domain $\Omega$. By construction, homogeneous Neumann boundary conditions $\partial_x u = 0$ are imposed on the artificial interface $\Gamma$.

Under this framework, taking the first distributional derivative of the piecewise field yields $\bar{u}_x = \overline{u_x} + [u]\delta_\Gamma$, where $[\cdot]_\Gamma$ denotes the jump operator across the interface $\Gamma$ applied to the assembled field, and $\delta_\Gamma$ is the Dirac delta distribution centered at $\Gamma$. Taking the distributional derivative a second time results in the identity
\[
\bar u_{xx}
=
\overline{u_{xx}}
...
[u]\delta_\Gamma',
\]
where $\delta_\Gamma'$ represents the distributional derivative of the Dirac delta. Since the Neumann interface condition enforces
\[
[\partial_xu]=0,
\]
this simplifies to
...
Because the Neumann interface condition holds at all times, the temporal derivative also satisfies $[\partial_x u_t]=0$, meaning the exact same distributional identity applies to the mixed viscous term:
\begin{equation*}
\bar u_{txx}
=
\overline{u_{txx}}
...
Consequently, the full viscoelastic wave equation over the decomposed domain becomes
\begin{equation*}
\bar u_{tt}
+
\gamma\bar u_t
...
Therefore, the independent subdomain solutions satisfy the original wave equation everywhere except at the interface $\Gamma$, where the missing physical coupling appears as a sum of two singular dipole distributions: a stiffness jump ($c^2[u]$) and a viscous jump ($\nu[u_t]$). Recovering the exact monolithic solution is therefore equivalent to explicitly restoring this combined singular interface source, effectively enforcing the continuity of the fields in the subdomains across $\Gamma$.

\subsection{A General Strang Splitting Formulation}

To implement the ARD coupling efficiently, we must first translate the continuous singular dipole sources into a discrete algebraic operator. We define the residual matrix
\begin{equation*}
C_{\mathrm{res}}
=
L_h
...
L_h^{(\mathrm N)},
\end{equation*}
where $L_h$ denotes the interior finite-difference Laplacian crossing the artificial interface, while $L_h^{(\mathrm N)}$ denotes the broken operator obtained by imposing homogeneous Neumann conditions on each subdomain (as introduced in \eqref{eq:boundary_corrected_laplacian}).

This relationship can be directly justified by recalling the distributional identity from \eqref{eq:dist_deriv}. In the discrete setting, applying the global finite-difference Laplacian $L_h \mathbf{u}$ natively evaluates the full distributional derivative $\bar{u}_{xx}$ by differencing across the interface. Conversely, the broken Neumann operator $L_h^{(\mathrm N)} \mathbf{u}$ acts entirely within the isolated subdomains, effectively evaluating only the piecewise classical derivative $\overline{u_{xx}}$. Taking the difference between these two discrete operators isolates the singular distribution:
\[
\underbrace{(L_h - L_h^{(\mathrm N)})}_{C_{\mathrm{res}}} \mathbf{u} \approx \bar{u}_{xx} - \overline{u_{xx}} = [u]\delta_\Gamma'.
\]
Consequently, the operator $C_{\mathrm{res}}$ natively acts as the exact discrete analog of the singular dipole distribution. Applying this residual matrix to the state variables numerically reconstructs the required localized interface forces.

With this operator defined, we cast the governing PDE as a first-order system in time. Defining the global state vector $\mathbf Z=[\mathbf u;\mathbf v]^T$, the full viscoelastic wave equation can be separated into two distinct evolution operators:
\begin{equation*}
\frac{d\mathbf Z}{dt}
=
\underbrace{
\mathcal{F}_{\mathrm{local}}(\mathbf Z)
}_{\text{Drift (Local Subdomains)}}
+
\underbrace{
\begin{bmatrix}
0\\
c^2C_{\mathrm{res}}\mathbf u+\nu C_{\mathrm{res}}\mathbf v
\end{bmatrix}
}_{\text{Kick (Interface Correction)}}.
\end{equation*}

The Drift operator $\mathcal{F}_{\mathrm{local}}(\mathbf Z)$ abstractly governs the uncoupled physics of the isolated subdomains under homogeneous Neumann boundary conditions, driven by the external physical source. We can treat the time integration of this local evolution as a generic black-box solver $\mathcal S_{\mathrm{local}}(\Delta t)$. 
The Kick operator exclusively applies the localized interface correction forces required to restore the global monolithic solution.

Because the interface correction depends on the instantaneous state, we decouple the operators using the symmetric Kick-Drift-Kick composition of Strang splitting (Strang~\cite{strang_construction_1968}),
\begin{equation*}
e^{\Delta t(A+B)}
=
e^{\frac{\Delta t}{2}B}
...
During the Drift step, the subdomains evolve independently using the black-box solver. During the Kick step, the positions $\mathbf u$ remain constant, while the velocities satisfy the localized interface ODE:
\begin{equation*}
\mathbf{\dot v}
=
\nu C_{\mathrm{res}}\mathbf v
...
Because $\mathbf u$ is constant during this substep, the interface force varies linearly with time only through $\mathbf v$. By evaluating the Kick step using an explicit forward Euler integration over a time interval $\tau$, we obtain the straightforward discrete update:
\begin{equation}
\mathbf u(t+\tau)=\mathbf u(t),
\qquad
\mathbf v(t+\tau)
=
\mathbf v(t)
+
\tau c^2C_{\mathrm{res}}\mathbf u(t)
+
\tau \nu C_{\mathrm{res}}\mathbf v(t).
\label{eq:kick_flow}
\end{equation}

Because the residual matrix $C_{\mathrm{res}}$ acts as a localized spatial second derivative, the explicit evaluation of the viscous term $\nu C_{\mathrm{res}}\mathbf v$ introduces a strict parabolic stability limit ($\Delta t \lesssim \Delta x^2 / \nu$). Consequently, while this explicit viscous Kick fully restores the physical continuity at the boundary, it forces the numerical scheme to operate at a lower Courant-Friedrichs-Lewy (CFL) number compared to the purely undamped wave equation.

\subsection{Application to FDTD}

To particularize this general framework for the FDTD method, the implicit local solver must satisfy two essential requirements: it must natively enforce homogeneous Neumann boundaries, and it must operate on (or be mathematically equivalent to) the first-order state space $\mathbf Z$ to allow for the instantaneous velocity updates prescribed by the Kick operator.

For the purely undamped wave equation, standard explicit leapfrog FDTD is mathematically equivalent to a first-order Velocity Verlet scheme, making the Strang-split ARD integration straightforward. However, for the viscoelastic wave equation, maintaining numerical stability necessitates an implicit or semi-implicit time-stepping scheme (such as Crank-Nicolson) for the mixed derivative $\mathbf u_{txx}$. While it is theoretically possible to construct a strictly localized, block-diagonal implicit FDTD Drift step, perfectly preserving the exact centered dispersion properties and stability limits of the monolithic FDTD scheme within a Strang-split, first-order framework introduces significant algorithmic complexity. Therefore, we omit the explicit FDTD particularization here and proceed directly to the pseudo-spectral discretization, where the state-space mapping is analytically exact.

\subsection{Application to PSTD}

The ARD construction extends naturally and optimally to the pseudo-spectral discretization. The principal advantage is that the interface operator is completely independent of the bulk discretization and can therefore be chosen at any desired finite-difference order, while the interior of each subdomain is solved spectrally.

For each subdomain, the solution is expanded in the DCT basis, yielding the uncoupled modal state vectors $\mathbf Z_{1,m}$ and $\mathbf Z_{2,m}$. The local Drift operator is simply the exact state-transition matrix $S_m(\Delta t)$ derived in the previous section \eqref{eq:modal_general_update}, incorporating the analytical propagation of the damped modal oscillators and the external physical forcing via the $T_m$ matrix.

The complete ARD-PSTD algorithm executes the following sequence at each time step $t_n$:

\begin{enumerate}

\item
\textbf{Interface reconstruction.}
The modal coefficients are transformed to the physical domain via the inverse DCT to obtain $\mathbf u^n$ and $\mathbf v^n$:
\begin{equation*}
    \mathbf{u}^n = \mathcal{D}^{-1} \{ \mathbf{U}_m^n \} \quad \text{and} \quad \mathbf{v}^n = \mathcal{D}^{-1} \{ \mathbf{V}_m^n \}.
\end{equation*}

\item
\textbf{First half-kick.}
The physical velocities are advanced over $\tau = \Delta t/2$ using the explicit Kick update \eqref{eq:kick_flow}. Because $C_{\mathrm{res}}$ acts locally in the physical domain, this evaluation is performed directly on the physical grid:
\begin{equation*}
    \mathbf{v}^{n+1/2} = \mathbf{v}^n + \frac{\Delta t}{2} \left( c^2 C_{\mathrm{res}} \mathbf{u}^n + \nu C_{\mathrm{res}} \mathbf{v}^n \right).
\end{equation*}
The updated velocities are then projected back into modal space via the forward DCT to obtain the intermediate modal states $\mathbf Z_m^{n+1/2}$:
\begin{equation*}
    \mathbf{V}_m^{n+1/2} = \mathcal{D} \{ \mathbf{v}^{n+1/2} \} \quad \text{and} \quad \mathbf{U}_m^{n+1/2} = \mathbf{U}_m^n,
\end{equation*}
yielding $\mathbf{Z}_m^{n+1/2} = [\mathbf{U}_m^{n+1/2}, \mathbf{V}_m^{n+1/2}]^T$.

\item
\textbf{Exact drift.}
The uncoupled modal equations are propagated analytically over a full time step $\Delta t$ with the exact state-transition matrix $S_m(\Delta t)$, yielding the updated intermediate modal states $\mathbf Z_m^*$:
\begin{equation*}
    \mathbf{Z}_m^* = S_m(\Delta t) \mathbf{Z}_m^{n+1/2}.
\end{equation*}

\item
\textbf{Interface reconstruction.}
The updated modal states $\mathbf Z_m^*$ are transformed back to the physical domain via the inverse DCT to obtain $\mathbf u^*$ and $\mathbf v^*$:
\begin{equation*}
    \mathbf{u}^* = \mathcal{D}^{-1} \{ \mathbf{U}_m^* \} \quad \text{and} \quad \mathbf{v}^* = \mathcal{D}^{-1} \{ \mathbf{V}_m^* \}.
\end{equation*}

\item
\textbf{Second half-kick.}
The physical velocities are advanced over the second half-step $\tau = \Delta t/2$ using the explicit Kick update \eqref{eq:kick_flow}:
\begin{equation*}
    \mathbf{v}^{n+1} = \mathbf{v}^* + \frac{\Delta t}{2} \left( c^2 C_{\mathrm{res}} \mathbf{u}^* + \nu C_{\mathrm{res}} \mathbf{v}^* \right).
\end{equation*}
Finally, the completed physical velocities are projected back into modal space to yield $\mathbf Z_m^{n+1}$:
\begin{equation*}
    \mathbf{V}_m^{n+1} = \mathcal{D} \{ \mathbf{v}^{n+1} \} \quad \text{and} \quad \mathbf{U}_m^{n+1} = \mathbf{U}_m^*,
\end{equation*}
yielding the fully synchronized state $\mathbf{Z}_m^{n+1} = [\mathbf{U}_m^{n+1}, \mathbf{V}_m^{n+1}]^T$.

\end{enumerate}

By leveraging the exact analytical solutions for the Drift (via the modal propagator) alongside the localized explicit interface Kick, the ARD-PSTD algorithm completely restores the global boundary coupling without requiring any global implicit matrix inversion, strictly preserving both the spectral accuracy of the bulk wavefield and the spatial locality of the interface forces.
...
The resulting stability limits reveal that the localized interface correction imposes only a mild restriction on the time step. Because the explicit correction operator involves the projection of global spectral modes onto a localized interface stencil, the exact maximum eigenvalue of the amplification matrix exhibits a minor dependence on the grid resolution $N$. This mesh-dependency arises from discrete spatial resonances between the highest-frequency global modes and the fixed interface stencil. To establish rigorous, mesh-independent stability bounds, a comprehensive numerical sweep of the amplification matrix $G$ (defined as the exact single-step transition matrix combining the modal propagator and interface correction) was performed across all grid resolutions up to $N=800$. The absolute global minimum (worst-case) admissible Courant numbers for second-, fourth-, sixth-, and eighth-order interface operators (referring to the spatial order of the finite-difference residual operator $C_{\text{res}}$) are strictly bounded by
\[
...
0.991,
\]
respectively. The monotonic decrease reflects the increasing spectral radius of the higher-order interface operators: wider interface stencils strengthen the explicit coupling between neighboring subdomains and therefore produce a slightly more restrictive stability condition. Remarkably, the eighth-order interface correction asymptotically locks into a worst-case limit of $0.991$, remaining exceptionally close to the classical FDTD stability limit while providing substantially higher spatial accuracy at the interface. It is worth noting that Courant numbers slightly greater than $1$ are possible because the CFL number is defined relative to the standard explicit FDTD stability limit, while the analytical modal propagator imposes no such intrinsic limit.

When the medium incorporates viscoelastic damping ($\nu > 0$), the stability criteria shift fundamentally. The exact explicit evaluation of the viscous interface forces strictly bounds the numerical scheme by a parabolic dimensionless limit
\[
S_{\max} = \frac{\nu \Delta t}{\Delta x^2}.
\]
A spectral analysis identical to the hyperbolic case isolates this purely diffusive stability restriction. For second-, fourth-, sixth-, and eighth-order interface operators, the maximum allowable diffusion numbers are rigorously bounded by
\[
S_{\max}
=
2.000,\;
...
1.233,
\]
respectively. Notably, the second-order operator perfectly recovers the exact theoretical maximum of $2.0$ expected from an explicit integration of a discrete diffusion equation, while the wider higher-order stencils systematically lower the maximum bound due to their expanded spatial support.
...
Conversely, when the time step slightly exceeds the theoretical limit, the instability becomes completely destructive across all spatial orders. Of course, it is obvious that violating the stability boundary will invariably lead to exponential divergence; this empirical parameter sweep over the perturbation $\epsilon$ simply serves to plausibly verify the exact numerically derived threshold. When integrated over the long $T=250$ timeframe, even the mildly unstable lower-order operators blow up: the fourth-order operator reaches an amplitude of $\approx 10^{50}$, and the second-order operator reaches $\approx 10^{126}$. The wider sixth- and eighth-order operators exhibit far more aggressive growth rates, with their numerical solutions exploding to the double-precision floating-point limit ($\approx 10^{303}$).

\subsubsection{Convergence of the Interface Correction Operator}

To evaluate the accuracy of the proposed explicit interface correction operator, we quantify the $L^\infty$ domain decomposition error, defined as
\begin{equation*}
E_{\infty} = \max_{j,n} \left| u_{j,n}^{\text{mono}} - u_{j,n}^{\text{ARD}} \right|,
\end{equation*}
as a function of the interface spatial order and grid resolution. The domain $\Omega=(0,L)$ is decomposed into two non-overlapping subdomains, and the ARD interface operator is applied with varying stencil sizes (Order 2, 4, 6, and 8). We evaluate the method across the six benchmark configurations defined in Section~\ref{sec:benchmark_problems}. For the propagating pulses (Configurations V and VI), they are placed asymmetrically at $x=L/4$ to prevent artificial symmetry at the boundary.
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_1/nodal_error_vs_monolithic.png} \caption{Smooth standing wave (undamped)} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_3/nodal_error_vs_monolithic.png} \caption{Smooth standing wave (damped)} \end{subfigure} \\
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_2/nodal_error_vs_monolithic.png} \caption{Non-smooth standing wave (undamped)} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_4/nodal_error_vs_monolithic.png} \caption{Non-smooth standing wave (damped)} \end{subfigure} \\
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_5/nodal_error_vs_monolithic.png} \caption{Smooth propagating pulse (undamped)} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_6/nodal_error_vs_monolithic.png} \caption{Non-smooth propagating pulse (undamped)} \end{subfigure}
...
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_1_order/nodal_error_vs_monolithic_order.png} \caption{Smooth standing wave (undamped)} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_3_order/nodal_error_vs_monolithic_order.png} \caption{Smooth standing wave (damped)} \end{subfigure} \\
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_2_order/nodal_error_vs_monolithic_order.png} \caption{Non-smooth standing wave (undamped)} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_4_order/nodal_error_vs_monolithic_order.png} \caption{Non-smooth standing wave (damped)} \end{subfigure} \\
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_5_order/nodal_error_vs_monolithic_order.png} \caption{Smooth propagating pulse (undamped) \\ ~} \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth} \centering \includegraphics[width=0.9\textwidth]{Images/pstd_snapshots/experiment_6_order/nodal_error_vs_monolithic_order.png} \caption{Non-smooth propagating pulse (undamped)} \end{subfigure}
...
\subsubsection{3D Numerical Experiments}

To demonstrate the capability of the proposed ARD solver in higher dimensions, we simulate the damped wave equation on a 3D computational domain composed of three interconnected rectangular subdomains. The physical dimensions are defined using a uniform spatial discretization step $\Delta h = 0.2$\,m. The three connected regions are parameterized by their grid offsets $(x_0, y_0, z_0)$ and grid dimensions $(N_x, N_y, N_z)$, yielding the sequence of connected subdomains depicted in Figure~\ref{fig:ard_3d_geometry}. 
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[scale=1.5,
        ...
        \node[anchor=west] at (5, 5.5) {$L_z = 2.0$\,m};
...
\end{figure}
This geometric configuration mimics a complex architectural layout. The physical parameters are set to a wave speed $c = 1$\,m/s and zero telegrapher damping ($\gamma = 0$). 

Instead of a non-zero initial condition, the system is excited from rest by a time-dependent Gaussian point source located at physical coordinates $(x_s, y_s, z_s) = (3.0, 5.0, 0.6)$\,m, which corresponds to grid index $(15, 25, 3)$ situated in the center of Subdomain 3. The time-varying injection signal is defined as
\[
S(t) = A \exp\left(-\pi^2 \left( \frac{2 \, \mathrm{CFL} \, t}{6} - 2.0 \right)^2 \right),
\]
where $\mathrm{CFL} = c \frac{\Delta t}{\Delta h}$ is the Courant--Friedrichs--Lewy (CFL) number and $A = 10^9$. 
To simulate an open environment, the outer boundaries of the domain are truncated using a Perfectly Matched Layer (PML) tuned for partial absorption with a reflection coefficient of $0.5$.

To evaluate the solver's response to frequency-dependent dissipation, we conduct the simulation across three different viscoelastic damping coefficients: $\nu \in \{2 \times 10^{-6}, 10^{-5}\}$. 

Figure~\ref{fig:ard_3d_snapshots} illustrates the temporal evolution of the 3D wavefield. Snapshots are captured at $t=0.04$\,s, $t=0.08$\,s, and $t=0.12$\,s for each damping configuration. As the wave propagates through the interconnected corridors, the ARD solver accurately resolves the interface transitions without introducing artificial reflections. Furthermore, the progressive attenuation and smoothing of the wavefronts scale consistently with the imposed viscoelastic parameter $\nu$, validating the accuracy of the multi-domain pseudospectral time-domain (PSTD) formulation in 3D.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.32\textwidth}
        \vspace{0pt}
        \centering
        \includegraphics[width=\textwidth]{Images/ard_3d/nu2_t04.png}
        \caption{$\nu = 2\cdot 10^{-6}$, $t=0.04$\,s}
    \end{subfigure}
    \hfill
    % ROW 2: t = 0.08s
    \begin{subfigure}[t]{0.32\textwidth}
    	\vspace{0pt}
    	\centering
    	\includegraphics[width=\textwidth]{Images/ard_3d/nu2_t08.png}
    	\caption{$\nu = 2\cdot 10^{-6}$, $t=0.08$\,s}
    \end{subfigure}
    % ROW 3: t = 0.12s
    \begin{subfigure}[t]{0.32\textwidth}
    	\vspace{0pt}
    	\centering
    	\includegraphics[width=\textwidth]{Images/ard_3d/nu2_t12.png}
    	\caption{$\nu = 2\cdot 10^{-6}$, $t=0.12$\,s}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.32\textwidth}
    	\vspace{0pt}
    	\centering
    	\includegraphics[width=\textwidth]{Images/ard_3d/nu10_t04.png}
    	\caption{$\nu = 10^{-5}$, $t=0.04$\,s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.32\textwidth}
    	\vspace{0pt}
    	\centering
    	\includegraphics[width=\textwidth]{Images/ard_3d/nu10_t08.png}
    	\caption{$\nu = 10^{-5}$, $t=0.08$\,s}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.32\textwidth}
    	\vspace{0pt}
    	\centering
    	\includegraphics[width=\textwidth]{Images/ard_3d/nu10_t12.png}
    	\caption{$\nu = 10^{-5}$, $t=0.12$\,s}
    \end{subfigure}
    
    \caption{3D ARD numerical experiment inside a multi-corridor geometry. The snapshots compare the wave propagation at three different time steps (columns) and two different viscoelastic damping configurations (rows). The solver seamlessly handles the internal boundaries between the three rectangular subdomains.}
    \label{fig:ard_3d_snapshots}
\end{figure}
...
Finally, we bridged the gap between forward numerical modeling and inverse problem solving by introducing a Differentiable Physics Neural Network (DPNN) architecture. By directly embedding the exact, differentiable PSTD solver into the training loop of a Sinusoidal Representation Network (SIREN), we successfully reconstructed unknown, non-smooth initial pressure fields while jointly recovering the macroscopic physical damping parameters from sparse sensor data. A mathematically rigorously derived Sobolev $H^1$ smoothness penalty gracefully suppressed high-frequency artifacts without smearing out sharp discontinuities, significantly outperforming purely data-driven black-box inversions.
...
This appendix derives the closed-form expressions of the matrices \(S_m\) and \(T_m\), respectively defined in \eqref{eq:Sm_def} and \eqref{eq:Tm_def}, associated with the exact time integration of the modal ordinary differential equations. For convenience, we recall their definitions:
\begin{equation*}
S_m=e^{A_m\Delta t},
\end{equation*}
\begin{equation*}
...
I would like to thank all the people who have been part of these years and who, in one way or another, helped me reach the end of this PhD.

First of all, I would like to thank my supervisors, Ilario Mazzieri, Gabriele Ciaramella and Fabio Antonacci. Thank you for always being available, for your support, and especially for being there during the more difficult moments of these years. I have learned a great deal from both of you, not only about research, but also about how to approach it.

I would also like to thank Martin J. Gander, who supervised me during my stay in Geneva. I learned a great deal from him about domain decomposition, but also about writing, presenting, and communicating mathematical ideas more clearly. I am particularly grateful for the time he dedicated to me and for everything I took away from that experience.

Desy, thank you for all the time we have spent together, and for all the time still to come. Through the easy moments and the difficult ones, you have always been someone with whom I could switch off, laugh, and simply enjoy being together. Your confidence in me and in what I can achieve has meant more to me than I can easily put into words.

To my parents, thank you for everything you have done for me throughout these years. Your support, both emotional and financial, has allowed me to pursue my studies without ever having to doubt whether you would be there for me. More than anything, I am grateful for the trust you have always placed in me.

To my siblings, Antonio Pio and Vittoria, thank you for always being part of my life and for the affection and support you have given me over the years. Even though you were not directly involved in my studies, having you around has made these years better.

Nonno Vincenzo, I have always admired your rationality, kindness, sense of humour, and humility. These are qualities I hope to carry with me as I grow older.

Nonna Vittoria, thank you for everything you have given me while growing up. I will always remember the time we spent together, especially our days by the sea, and, of course, your cooking.

Nonno Antonio, I have learned a lot from the way you approach your work, your team, and your own abilities. Your confidence in what you can do taught me that ambition and self-belief are not things to hide, but things that should be cultivated.

Nonna Pina, thank you for all the love and affection you have given me throughout my life. Your presence has always been an important part of my idea of family.

To Giovanni, Alessandro, and Marco, thank you for all these years of friendship. From the earliest memories to the journeys, evenings out, and countless moments of laughter, you have always been there, and I hope there are many more memories still to make together.

To Marco, Simone, and Guglielmo, I also owe a special thanks for the years we spent together during our undergraduate studies. Between Neapolitan songs, nights out, and everything in between, we created some of the memories I will remember most fondly from that period.

Jacopo, thank you for being a worthy academic rival. Competing with you, comparing ourselves, and pushing each other to do better made me grow more than I probably realised at the time.

Finally, I would like to thank my colleagues at the MOX laboratory. Thank you for the conversations, the jokes, and the laughs during lunch in Sala Mella. Those everyday moments made the routine of research much more enjoyable.
