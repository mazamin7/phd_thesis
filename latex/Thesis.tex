...
In this section, we derive and analyze a FDTD scheme for the damped wave equation, following the approach described in \cite{bilbao_numerical_2009}, Chapter 6, \S6.2. For simplicity, we derive the method in one spatial dimension. The extension to higher dimensions is obtained by replacing the second spatial derivative by the corresponding discrete Laplacian \cite[Chapter 7]{bilbao_numerical_2009}.
...
By collecting the unknowns at all grid points $i=0,\dots,N$ into the global state vectors $\mathbf{u}^n = [u_0^n, \dots, u_N^n]^T$, the interior update equations for $i=1,\dots,N-1$, together with the discrete boundary equations evaluated at $i=0$ and $i=N$, can be assembled into the global algebraic system:
\begin{equation}
...
\end{equation}
where the matrices $A_{\mathrm{bc}},B_{\mathrm{bc}}$, and $C_{\mathrm{bc}}$ coincide with the interior matrices except for their first and last rows, which are populated by the coefficients of the specific boundary scheme, and the vector $\mathbf{b}_{\mathrm{bc}}^{\,n}$ collects the contribution of the prescribed boundary data.
