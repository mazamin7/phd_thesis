...
\begin{equation} \label{eq:modal_state_def}
\mathbf Z_m(t)
=
\begin{bmatrix}
U_m(t)\\
...
\begin{equation} \label{eq:modal_ode_first_order}
...
This relationship can be directly justified through the theory of distributions. For a piecewise smooth function $u$ exhibiting a jump $[u]$ across the interface $\Gamma$ while preserving continuity of traction ($[u_x]=0$), its distributional second derivative is given by
\[
u_{xx} = \{u_{xx}\} + [u]\delta_\Gamma',
\]
where $\{u_{xx}\}$ denotes the classical piecewise second derivative evaluated strictly within the subdomains.
In the discrete setting, applying the global finite-difference Laplacian $L_h \mathbf{u}$ natively evaluates the full distributional derivative $u_{xx}$ by differencing across the interface. Conversely, the broken Neumann operator $L_h^{(\mathrm N)} \mathbf{u}$ acts entirely within the isolated subdomains, effectively evaluating only the piecewise classical derivative $\{u_{xx}\}$. Taking the difference between these two discrete operators isolates the singular distribution:
\[
(L_h - L_h^{(\mathrm N)}) \mathbf{u} \approx u_{xx} - \{u_{xx}\} = [u]\delta_\Gamma'.
\]
...
The physical velocities are advanced over $\tau = \Delta t/2$ using the explicit Kick update \eqref{eq:kick_flow}. Because $C_{\mathrm{res}}$ acts locally in the physical domain, this evaluation is performed directly on the physical grid. The updated velocities are then projected back into modal space via the forward DCT to obtain the intermediate modal states $\mathbf Z_m^{n+1/2}$.
...
The physical velocities are advanced over the second half-step $\tau = \Delta t/2$ using the explicit Kick update \eqref{eq:kick_flow}. Finally, the completed physical velocities are projected back into modal space to yield $\mathbf Z_m^{n+1}$.
