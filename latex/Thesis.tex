...
Let $u$ denote the exact continuous solution and let $\bar u$ denote the piecewise field obtained by solving independently on $\Omega_1$ and $\Omega_2$. By construction, homogeneous Neumann boundary conditions $\partial_x u = 0$ are imposed on the artificial interface $\Gamma$.
...
[u]\delta_\Gamma'
\]
holds. Here, $[\cdot]_\Gamma$ denotes the jump operator across the interface $\Gamma$, while $\delta_\Gamma$ and $\delta_\Gamma'$ represent the Dirac delta distribution centered at $\Gamma$ and its distributional derivative, respectively. Since the Neumann interface condition enforces
\[
[\partial_xu]=0,
\]
this simplifies to
...
Because the Neumann interface condition holds at all times, the temporal derivative also satisfies $[\partial_x\dot u]=0$, meaning the exact same distributional identity applies to the mixed viscous term:
\begin{equation*}
\bar u_{txx}
=
\overline{u_{txx}}
