...
Notice that the viscoelastic damping term $\nu u_{txx}$ has been intentionally omitted from this FDTD formulation. In a finite-difference setting, resolving the mixed spatio-temporal derivative $u_{txx}$ requires an implicit or semi-implicit time-stepping scheme to maintain numerical stability. Advancing such an implicit scheme requires the global inversion of the spatial Laplacian matrix at every time step. Because matrix inversion transforms a sparse, local operator into a dense, globally coupled one, the localized interface perturbation generated by $C_{\mathrm{res}}$ would instantaneously propagate across all degrees of freedom in the computational domain. This dense global coupling intrinsically destroys the locality of the correction, meaning that the ARD methodology---which relies entirely on the explicit localization of interface forces---cannot be applied to implicitly damped FDTD schemes without losing its fundamental efficiency.
...
Recalling the modal state vector $\mathbf Z_m$ and its uncoupled first-order evolution introduced in the pseudo-spectral discretization \eqref{eq:modal_ode_first_order}, the governing equation extended with the interface correction becomes
\begin{equation}
\frac{d\mathbf Z_m}{dt}
=
\underbrace{
...
\end{equation}
where $A_m$ is the uncoupled damped modal oscillator matrix defined in \eqref{eq:modal_ODE_matrix_A}, $\mathbf W_m(t)$ is the external physical forcing term, and $F_m(U)$ denotes the modal representation of the interface correction.

The Drift operator represents the uncoupled damped modal oscillators driven by the external physical source, whose flow is known analytically through the exact state-transition matrix \eqref{eq:modal_general_update}. The Kick operator contains only the interface forcing. During the evolution under the Kick, the modal positions remain fixed,
\[
\dot U_m=0,
\]
...
\item
\textbf{Exact drift.}
Using the updated modal velocities, the uncoupled modal equations are propagated analytically over a full time step with the exact state-transition matrix (including the exact application of the external physical forcing), yielding the updated modal positions
\[
U_{1,2,m}^{\,n+1}
\]
