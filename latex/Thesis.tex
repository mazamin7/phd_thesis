...
Figure~\ref{fig:ard_convergence_dx} and Figure~\ref{fig:ard_convergence_order} present the $L^\infty$ domain decomposition error relative to the monolithic PSTD reference solution as a function of the grid resolution $\Delta x$ and the spatial order, respectively. For the smooth standing wave and smooth Gaussian pulse, increasing the spatial order of the correction operator yields dramatically improved convergence rates. However, for non-smooth profiles (such as the discontinuous trapezoidal wave and triangular pulse), the high-frequency spectral interpolation noise dominates the error at the interface. In these non-smooth regimes, the higher-order correction stencils provide diminishing returns compared to the baseline second-order operator.
