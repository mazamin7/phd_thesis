...
As observed in Figure~\ref{fig:ard_convergence_dx}, the domain decomposition error converges at approximately second-order ($O(\Delta x^2)$) for the smooth standing waves (a, c) and the propagating pulses (e, f). In contrast, the non-smooth standing waves (b, d) exhibit a degraded, approximately first-order convergence. 

Furthermore, Figure~\ref{fig:ard_convergence_order} demonstrates that for non-smooth profiles, increasing the spatial order of the correction operator is of little help. While higher-order stencils yield a lower error, they do not converge exponentially; for instance, the negative logarithm ($-\log_{10}$) of the error only marginally improves from approximately $3.8$ at order 2, to $3.0$ at order 4, $2.9$ at order 6, and $2.85$ at order 8. In these non-smooth regimes, the high-frequency spectral interpolation noise dominates the error at the interface. Conversely, for smooth profiles, higher-order correction stencils yield dramatically improved, exponential convergence rates.
