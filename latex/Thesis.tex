...
As observed in Figure~\ref{fig:ard_convergence_dx}, when the interface correction operator is fixed at spatial order 2, the domain decomposition error converges linearly ($O(\Delta x)$) across almost all configurations. Crucially, this first-order convergence rate applies equally to both smooth (a, b) and non-smooth (c, d, f) profiles. This demonstrates that the localized, low-order explicit interface correction acts as a numerical bottleneck, strictly limiting the global spatial accuracy independently of the underlying wavefield's smoothness. (The smooth propagating pulse in (e) reaches the numerical error floor of the monolithic baseline early on, obscuring further convergence).

Furthermore, Figure~\ref{fig:ard_convergence_order} illustrates the effect of increasing the spatial order of the correction operator for a fixed grid resolution. For smooth profiles (a, b, e), increasing the order yields a rapid, exponential reduction in the domain decomposition error. Conversely, for non-smooth profiles (c, d, f), employing higher-order interface stencils is detrimental; rather than converging, the domain decomposition error actually increases. This degradation occurs because wider, higher-order explicit stencils inherently amplify the high-frequency spectral interpolation noise (Gibbs phenomenon) triggered by the discontinuities at the subdomain boundaries.
